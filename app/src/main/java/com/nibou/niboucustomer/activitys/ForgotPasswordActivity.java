    ...
    private void forgotPasswordNetworkCall() {
        ...
        ApiHandler.requestService(context, ApiClient.getClient().create(ApiEndPoint.class).forgetPasswordNetworkCall(parameters), new ApiHandler.CallBack() {
            ...
            public void success(boolean isSuccess, Object data) {
                ...
                if (isSuccess) {
                    AppDialogs.getInstance().showCustomDialog(context, getString(R.string.success).toUpperCase(), getString(R.string.dialog_reset_pwd_message), getString(R.string.OK), getResources().getColor(R.color.green), status -> {
                        onBackPressed();
                    });
                } else {
